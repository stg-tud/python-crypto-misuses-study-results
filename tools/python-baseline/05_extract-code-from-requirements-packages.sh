...
input=$1
counter=1
destination="/media/ubuntu/DATA/repositories-python-requirements-code/"
...
then
    ...
    while read line; do
        ...
        then
            echo "$counter $line"
            date >> extract-code-from-requirements-packages-progress.log
            echo "$counter $line" >> extract-code-from-requirements-packages-progress.log
            if [[ $line == *.tar.gz ]]; then
                ...
                date >> extract-code-from-requirements-packages.log
                echo $line >> extract-code-from-requirements-packages.log
            fi
            counter=$[$counter +1]
        fi
    done <$input
